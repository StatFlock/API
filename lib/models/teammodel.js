...
{
    var query = 'SELECT * FROM statflock WHERE type="team" AND fullName LIKE "' + searchString + '%"';
...
{
    var query = 'SELECT * FROM statflock WHERE type="team" AND sport = "' + sport + '"';
    var n1qlQuery = N1qlQuery.fromString(query);
    db.query(n1qlQuery, function(err, result) {
        if (err) {
            console.log('query failed', err);
...
{
    var query = 'SELECT * FROM statflock WHERE type="team" AND sport = "' + sport + '" AND league =  "' + league + '"';
    var n1qlQuery = N1qlQuery.fromString(query);
    db.query(n1qlQuery, function(err, result) {
        if (err) {
            console.log('query failed', err);
...
{
    var query = 'SELECT * FROM statflock WHERE type="team" AND sport = "' + sport + '" AND league =  "' + league + '" AND division =  "' + division + '"';
    var n1qlQuery = N1qlQuery.fromString(query);
    db.query(n1qlQuery, function(err, result) {
        if (err) {
            console.log('query failed', err);
