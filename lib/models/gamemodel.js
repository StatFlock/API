...
GameModel.getGamesBySportDate = function (date, sport, callback)
{
    ...
    var query = 'SELECT * FROM statflock WHERE type="game" AND sport = "' + sport + '" AND gameDate =  "' + date + '"';
...
{
    var query = 'SELECT * FROM statflock WHERE type="game" AND sport = "' + sport + '" AND gameDate =  "' + date + '" AND league =  "' + league + '"';
    var n1qlQuery = N1qlQuery.fromString(query);
    db.query(n1qlQuery, function(err, result) {
        if (err) {
            console.log('query failed', err);
